"""
Pathological Gait Anomaly Detection — Transformer Training Script

Trains the advanced Patch-based Transformer Autoencoder on normal walking data.
This script serves as the direct comparison to `train_anomaly.py` (LSTM).
...
from src.models.transformer import PatchTransformerAutoencoder
...
# Set separate directories for transformer results
...
def main():
    ...
    print("  Advanced Patch-based Transformer Autoencoder (2024)")
    print("=" * 60)
    print(f"\nDevice: {DEVICE}")
    print(f"Seed: {SEED}")
    ...
    # We will save the transformer model with a different name
    # We need to hack the trainer briefly to use a different filename or just accept the override
    # Since AnomalyTrainer hardcodes 'best_model.pt', we'll temporarily override its save method
    # or just rename it afterwards. Actually AnomalyTrainer._save_checkpoint takes filename.
    # We'll just patch the hardcoded names in the loop for safety if possible,
    # but the easiest is to just let it save as best_transformer.pt
    
    # Let's override the checkpoint names inside train method dynamically:
    original_save = trainer._save_checkpoint
    original_load = trainer._load_checkpoint
    
    def custom_save(filename, epoch, val_loss):
        original_save("best_transformer.pt", epoch, val_loss)
        
    def custom_load(filename):
        original_load("best_transformer.pt")
        
    trainer._save_checkpoint = custom_save
    trainer._load_checkpoint = custom_load
    ...
    # Override save paths for plots
    
    print("\nPlotting training history...")
    plot_training_history(history, save_path=TRANSFORMER_RESULTS_DIR / "transformer_training_history.png")
    
    print("Plotting error distribution...")
    plot_reconstruction_error_distribution(
        train_errors, val_errors, trainer.anomaly_threshold, test_errors,
        save_path=TRANSFORMER_RESULTS_DIR / "transformer_error_distribution.png"
    )
    ...
    plot_reconstruction_examples(
        ...
        save_path=TRANSFORMER_RESULTS_DIR / "transformer_reconstruction_examples.png"
    )
    ...
    plot_gait_symmetry_features(
        ...
        save_path=TRANSFORMER_RESULTS_DIR / "transformer_symmetry_features.png"
    )
    ...
    print("=" * 60)
    print(f"\nResults saved to: {TRANSFORMER_RESULTS_DIR}")
    print(f"Best model saved to: {CHECKPOINT_DIR / 'best_transformer.pt'}")
